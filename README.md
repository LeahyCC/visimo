...
scene ──► history[current] ──► bright ──► blur x3 ──► composite ──► canvas
                ▲ add                                     ▲
                └── history[other], carried along the scene's flow,
                    then zoomed, turned, decayed, held under a ceiling
                         ▲
...
Each stage has an `enabled` flag and the stack has its own; a stage runs only when both are on. `post/params.ts` holds the defaults and `writePostUniform`, which resolves every toggle on the CPU and writes the 128-byte uniform each pass reads, so the shaders have no branches. The last sixteen of those bytes are the flow block, which a scene's `cover` reaches through `writePostUniform`'s one optional argument. With every stage off the composite is a straight copy.
