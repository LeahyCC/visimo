...
`from` is one of twenty-six packet fields, plus `lowEnd` for the louder of sub and bass, plus `tension` and `presence`, which are not in the packet: the resolver takes tension as an argument and presence is the director's fade. Five of the packet fields are the song-scale ones above, which move over tens of seconds rather than per frame, three are the harmony, two the structure and two the beat as a clock. Five are the per-band pulses, `subPulse` through `treblePulse`. The raw per-band hits are not among them, for the reason `onset` is not: they are events rather than levels, and an implementation reads an event straight from the packet. `to` is a knob of that study's own implementation and nothing else, so two studies in one cast cannot fight over the same number by accident. `curve` is `linear`, `square`, `sqrt` or `invert`, all of which keep 0 at 0 and 1 at 1 except the last. `gain` may be negative, which is how a feature thins a number rather than raising it: `treble → viscosity` at −0.18 is the fluid keeping its detail when the music is busy. Several rows may name the same knob and they add.
