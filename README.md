...
- **Grade.** The frame's edges and its colour, in the composite between the bloom and the tonemap, so it costs no pass of its own and the tonemap still bends what the vignette leaves over 1. Two numbers, both neutral at rest, so a look that switches the stage on and says nothing changes nothing.
...
Each stage has an `enabled` flag and the stack has its own; a stage runs only when both are on, and what is on is decided by what is live: the canvas switches the feedback on, a ribbon ink switches the ribbon on, and the looks switch on the stages they name. `post/params.ts` holds the defaults and `writePostUniform`, which resolves every toggle on the CPU and writes the 192-byte uniform each pass reads, twelve vec4s, so the shaders have no branches. The last eighty of those bytes are the flow block, the floor, the ribbon's two vec4s and the grade, in that order, so every float before them is where it always was; the flow's `cover` reaches the first of them through `writePostUniform`'s one optional argument. A ribbon that is off, or has no light or no width, writes all eight of its floats as zero, and its pass is not run at all. With every stage off the composite is a straight copy.

`postSummary` names the stages that are running in pass order, `ribbon` first when a cast holds that ink, and that line is what the canvas's `data-post` prints. Only Drift and Melt hold it, so Plume, Wash and Prism print exactly what they did before the stage existed. `grade` sits between `chroma` and `tonemap` and is off unless a look names it: none of the five shipped casts' looks does, so all five print what they always did, and only `squeeze` prints it.
...
```json
{
  ...
  "look": "warm-soft",
  "canvas": {
    "knobs": { "feedback.amount": 0.3, "feedback.decay": 0.68 },
    "mapping": [{ "from": "swell", "to": "feedback.decay", "gain": 0.04 }]
  },
  "overrides": {
    "dye-plumes": {
      "knobs": { "dye": 3, "colourShift": 0.5 },
      "mapping": [{ "from": "energy", "to": "dye", "gain": 2.3 }]
    }
  }
}
```
...
Each of the five resolves to the numbers its 0.1 preset did, which `src/studies/cast.test.ts` and `src/gpu/Renderer.test.ts` both check against `casts/preset-frames.json`, a capture of the preset path's own output taken before it was deleted. The first proves the studies layer lands on those numbers; the second drives real frames and proves the renderer hands each of them to the right implementation. Both walk the knobs the capture recorded, so a knob added since is invisible to them, and nothing regenerates that file: it is a record of what shipped rather than a list of what exists. A knob added since is pinned by a test of its own beside them, which is where the fractal's `glint` is held to 0 for Prism and to Melt's own number for Melt.

| Preset | Cast                                                 | What it does differently                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| ------ | ---------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| Plume  | lazy fluid, dye plumes, warm and soft                | The default, and the one the language below describes. Five emitters well split by voice, the key colouring all of them, bass deciding what an onset is worth and treble the vorticity, and the song's own pace, swell and weight moving the resting numbers.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| Wash   | turbulent fluid, dye plumes, warm and soft           | The opposite way round, with thinner viscosity, wider spread, a slower orbit and the voices half blended, so the plumes stay separate instead of growing into each other. It takes the key's colour and nothing else of the song's shape, and it went almost black through the loudest drop until energy fed the dye as well as its decay.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
| Drift  | lazy fluid, dye plumes, ribbon, warm and soft        | Plume with the trail turned up and carried by the fluid itself: the last frame is read back along the velocity field, so the echo swirls with the dye rather than zooming out of the middle. The zoom breathes about 1 and the turn only moves with a chord, the dye and the intensity are pulled down against the longer sum, energy and swell lengthen the trail through `feedback.decay` and raise the floor with it, a floor of 0.018 holds the background near black against the fluid's own base colour, and a ceiling of 1.8 keeps it off white, tightening to 1.25 in a loud, hard drop. The ribbon is a line across the middle.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
...
Plume is written so that each thing the extractor knows about the song moves one thing a viewer can see, and only that. This is the language; a cast is free to speak another. The post rows carry Plume's numbers; Wash, Drift, Melt and Prism say the same things with gains scaled to their own resting values, and skip the rows for a stage they switch off.
...
| A passage lifting or dropping   | `swell`                                | `spread` +0.2, `dyeDecay` −0.18, `feedback.decay` +0.06                        | A drop fills the frame and its dye lingers; a breakdown pulls in and thins. A drop lengthens the trails and a breakdown shortens them.                                                               |
...
| Loud now                        | `energy`                               | the decays, `spread`, `force`, `dye`, `feedback.carry` +0.4                    | The frame fills and clears with the level. The echo is dragged along the fluid harder as it gets loud.                                                                                               |
...
| The beat                        | `beatPulse`                            | `intensity` +0.5, `chromatic.beat`, `bloom.intensity` +0.12                    | A flash, a split and a swell of glow on the beat.                                                                                                                                                    |
| Where in the beat we are        | `beatPhase`, inverted                  | `feedback.zoom` +0.004                                                         | The trails breathe out on the beat and settle before the next. Rows only add, so a wrong tempo cannot be gated off by `tempoConfidence`; the gain is small enough that a wrong one is a slow breath. |
...
| Bass-led, on the ribbon         | `weight`                               | `ribbon.width` +1.5                                                            | In Drift and Melt, the two casts that hold the ribbon: a heavy track draws a thicker line.                                                                                                           |
...
| character                          | The five axes. Each writes the packet row the director reads it from: `weight`, `hardness`, `keyClarity` for tonality, `tempoConfidence` for steadiness, and drive writes both `pace` and `tempo`, which is what the reader averages. Studies read those rows, so a study sees the same numbers the director does.                                                             |
...
**The names that survive.** `PRESETS`, `DEFAULT_PRESET_ID`, `findPreset`, `presetOrDefault` and `stepPreset` all still exist in `visimo/presets` and all hold pinned casts, so a picker written against 0.1 needs no change beyond the type it names. `CASTS`, `DEFAULT_CAST_ID`, `findCast`, `castOrDefault` and `stepCast` are the same five under their own names.
