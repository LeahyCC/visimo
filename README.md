...
Each stage has an `enabled` flag and the stack has its own; a stage runs only when both are on, and what is on is decided by what is live: the canvas switches the feedback on, a ribbon ink switches the ribbon on, and the looks switch on the stages they name. `post/params.ts` holds the defaults and `writePostUniform`, which resolves every toggle on the CPU and writes the 176-byte uniform each pass reads, so the shaders have no branches. The last sixty-four of those bytes are the flow block, the floor and the ribbon's two vec4s, in that order, so every float before them is where it always was; the flow's `cover` reaches the first of them through `writePostUniform`'s one optional argument. A ribbon that is off, or has no light or no width, writes all eight of its floats as zero, and its pass is not run at all. With every stage off the composite is a straight copy.

`postSummary` names the stages that are running in pass order, `ribbon` first when a cast holds that ink, and that line is what the canvas's `data-post` prints. Only Drift and Melt hold it, so Plume, Wash and Prism print exactly what they did before the stage existed.
