...
Each frame: read the analyser through the feature client, stamp time and dt into the CPU packet, resolve the preset's mapping into the scene's numbers and the stack's, step the preset's flow if it asked for one, run the scene's passes into the stack's texture, run the stack onto the canvas (handing it the client's waveform for the ribbon), then draw the HUD if it is on. Each scene uploads its own resolved parameters. Nothing per frame touches React.
...
```
the analyser's waveform ──► ribbon ──┐ add
                                     ▼
...
- **Ribbon.** The sound itself drawn as a line, into the scene's texture after the scene and before the feedback pass, so it is fresh light like the scene's and the trails carry it into sheets. Off by default, and a preset that switches it on still draws nothing until `ribbon.intensity` is above 0. It is one triangle strip with two vertices a point, one either side of the line, so the width is a real number of pixels: a line list is one pixel wide on every GPU and would vanish on a 4K canvas. `ribbon.width` is written for a 1080 pixel canvas and scaled by the canvas's short side, so 4K draws it twice as wide, and the edge fades over a pixel so it is soft without being wider than asked. The blend is additive, and leaves alpha alone.
  ...
  - **The WebGL2 fallback skips it.** It wants the waveform and a strip drawn into the scene's target, and that path has neither. A preset that turns it on there still draws, without the line, and nothing on that path reads its numbers. `data-post` still names the stage, as it names the feedback there whose carry is skipped too.
...
Each stage has an `enabled` flag and the stack has its own; a stage runs only when both are on. `post/params.ts` holds the defaults and `writePostUniform`, which resolves every toggle on the CPU and writes the 176-byte uniform each pass reads, so the shaders have no branches. The last sixty-four of those bytes are the flow block, the floor and the ribbon's two vec4s, in that order, so every float before them is where it always was; a scene's `cover` reaches the first of them through `writePostUniform`'s one optional argument. A ribbon that is off, or has no light or no width, writes all eight of its floats as zero, and its pass is not run at all. With every stage off the composite is a straight copy.

`postSummary` names the stages that are running in pass order, `ribbon` first when a preset has it on, and that line is what the canvas's `data-post` prints. The ribbon is off in every preset that does not ask for it, so Plume, Wash and Prism print exactly what they did before the stage existed, and only Drift and Melt, which turn it on, print `ribbon` as well.
...
`from` is one of thirty packet fields, plus `lowEnd` for the louder of sub and bass. Five of them are the song-scale ones above, which move over tens of seconds rather than per frame, three are the harmony, two the structure, two the beat as a clock and four the moment. Five are the per-band pulses, `subPulse` through `treblePulse`. The raw per-band hits are not among them, for the reason `onset` is not: they are events rather than levels, and a scene reads an event straight from the packet. `impact` is an event and is among them anyway, because it is published as a pulse rather than as a flag: it is 1 on the frame the drop lands and falls away in a fifth of a second, so a row on it reads like a row on `beatPulse` and fires once a drop rather than once a beat. `to` is one of the scene's own knobs, or a dotted post target such as `bloom.intensity`; that is how the parser tells the two apart. `curve` is `linear`, `square`, `sqrt` or `invert`, all of which keep 0 at 0 and 1 at 1 except the last. `gain` may be negative, which is how a feature thins a number rather than raising it: `treble → viscosity` at −0.18 is the fluid keeping its detail when the music is busy. Several rows may name the same knob and they add.
...
Nothing on screen is static, in every preset and every study added later. Each post stage a preset switches on has at least one `audioMapping` row aimed at it, and its scene is mapped too. `src/presets/postMapping.test.ts` walks `PRESETS` and fails, naming the preset and the stage, when a stage is on and nothing drives it. The same file resolves every preset against a silent packet, a full one and three values of `swell`, keeps every lane inside a safe range, holds the feedback gain under 1, and asserts that a full packet leaves `intensity`, `saturation` and `tonemap.exposure` at or below rest: a loud, hard passage puts its force into motion and structure, and pulls the light back rather than adding more. A new preset is covered without touching the test.
...
| Drift  | Fluid        | Plume with the trail turned up and carried by the fluid itself: the last frame is read back along the velocity field, so the echo swirls with the dye rather than zooming out of the middle. The zoom breathes about 1 and the turn only moves with a chord, the dye and the intensity are pulled down against the longer sum, energy and swell lengthen the trail through `feedback.decay` and raise the floor with it, a floor of 0.018 holds the background near black against the fluid's own base colour, and a ceiling of 1.8 keeps it off white, tightening to 1.25 in a loud, hard drop. The waveform ribbon is on as a line across the middle, at an intensity of 0.15 with `energy` adding 0.3, so the trail carries the sound as sheets.                                                                                                                                                                                                                                                                                                                                                                                       |
...
| Melt   | Kaleidoscope | Prism carried by a fluid the music pushes: the scene solves no field, so the preset asks for one and the renderer runs a fluid under it that is stepped and never drawn. The trail has a gain of 0.95, a little under a second to fall to a tenth, the carry is 1 so every part of the screen bends its own way, a ceiling of 1.2, tightening to 0.94 in a loud, hard drop, holds the filled areas, a floor of 0.03 cuts the dim body of the fractal out of the trail so only its bright ridges smear, and the scene's intensity is 0.5 against Prism's 1 so fresh light and carried light together do not wash out. These were set by eye on a real track: the first cut, a gain of 0.972 under a floor of 0.0012, let the fractal fill every pixel every frame and the whole screen settled into one flat wash at the ceiling. The waveform ribbon is on as a circle about the middle, its radius the sound, at an intensity of 0.3 with `energy` adding 0.4, which the fluid then smears with the fractal. Under WebGL2 there is no compute and so no flow, and no ribbon either; it draws as Prism with the same trail sitting still. |

### What means what

Plume is written so that each thing the extractor knows about the song moves one thing a viewer can see, and only that. This is the language; a preset is free to speak another. The post rows carry Plume's numbers; Wash, Drift, Melt and Prism say the same things with gains scaled to their own resting values, and skip the rows for a stage they switch off.
...
| Bass-led, on the ribbon         | `weight`                               | `ribbon.width` +1.5                                                            | In Drift and Melt, which switch the ribbon on: a heavy track draws a thicker line.                                                                                                                   |
