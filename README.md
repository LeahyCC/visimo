...
The renderer's side of it is one method, `renderer.setBench(bench | null)`, separate from `setPreset`. A `Bench` is `{ live, frame?, waveform? }`: a `LiveCast` to draw in place of the pinned cast or the director's, a function called once per drawn frame with the packet and the real `dt` after the packet is written and before anything reads it, and samples for the ribbon. While one is set it stands over both, the director is still stepped on the packet it left behind, and clearing it hands the picture back to whatever was there. A new cast empties the canvas, so a study is judged on its own trails; the same cast handed again does not. It is a development hook and a host has no use for it. `window.visimo` in the dev build is unchanged, since the demo imports the renderer directly.
