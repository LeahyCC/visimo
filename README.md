...
Each of the five resolves to the numbers its 0.1 preset did, which `src/studies/cast.test.ts` and `src/gpu/Renderer.test.ts` both check against `casts/preset-frames.json`, a capture of the preset path's own output taken before it was deleted. The first proves the studies layer lands on those numbers; the second drives real frames and proves the renderer hands each of them to the right implementation.
